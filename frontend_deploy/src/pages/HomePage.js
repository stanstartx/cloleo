...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen overflow-hidden home-premium-gradient" data-testid="home-page">
      ...
      {/* Trending Products - Featured Showcase */}
      <motion.section 
        ref={trendingRef}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[1.4fr_1fr] gap-6">
              ...
              <div className="grid gap-6">
                {trendingProducts[4] && (
                  <Link
                    to={`/produit/${trendingProducts[4].id}`}
                  ...
                  >
                    <div className="relative overflow-hidden bg-slate-100">
                      <img
                        src={trendingProducts[4].images?.[0] || trendingProducts[4].main_image || 'https://via.placeholder.com/600'}
                        alt={trendingProducts[4].name}
                    ...
                    <div className="p-5">
                      ...
                      <h3 className="mt-4 text-xl md:text-2xl font-bold text-slate-900">{trendingProducts[4].name}</h3>
                      <p className="mt-3 text-sm leading-6 text-slate-600 line-clamp-3">{trendingProducts[4].description || 'Produit populaire mis en avant pour cette semaine.'}</p>
                      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <span className="text-2xl font-bold text-slate-900">{(trendingProducts[4].promo_price_fcfa || trendingProducts[4].price_fcfa || trendingProducts[4].price || 0).toLocaleString()} FCFA</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {trendingProducts.slice(5, 7).map((product, index) => (
                    <Link
                      key={product.id}
                      to={`/produit/${product.id}`}
